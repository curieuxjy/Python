# Python <img align="left" width="15%" height="15%" src="./assets/images/python2.png">
> Everything [I](https://github.com/curieuxjy) do with Python

**Author** : Jungyeon Lee ([curieuxjy](https://github.com/curieuxjy))

<p align="center">
  <img width="100%" height="100%" src="./assets/images/python4.png">
</p>

### How to use this repository
Download the repository using `git clone` or `.zip` file and STUDY!

## Contents
|Folder name|Brief introduction & tag|book|lecture|project|
|-----------|------------------|----|-------|-------|
|[AI_expert_oss](./AI_expert_oss/README.md)|Object detection/U-Net/CV| |✔️| |
|[Data_Analysis](./Data_Analysis/README.md)|파이썬 라이브러리를 활용한 데이터 분석|✔️| | |
|[Data_preprocessing](./Data_preprocessing/README.md)|데이터 전처리 대전/SQL/R|✔️| | |
|[Developer_python](./Developer_python/README.md)|개발자를 위한 파이썬|✔️|✔️| |
|[Effective_python](./Effective_python/README.md)|2nd edition|✔️| | |
|[Elice_nipa](./Elice_nipa/README.md)|제조IoT 공정 데이터/ML| |✔️|✔️|
|[Flask](./Flask/README.md)|점프 투 플라스크|✔️| | |
|[Lecture_code](./Lecture_code/README.md)|Python beginner를 위한 강의 코드| | |✔️|
|[Quiz_code](./Quiz_code/README.md)|Python beginner를 위한 퀴즈 코드| | |✔️|
|[Machine_learning](./Machine_learning/README.md)|brief introduction|✔️| | |
|[Open_data_analysis_oss](./Open_data_analysis_oss/README.md)|공개 데이터 분석/OSS| |✔️|✔️|
|[Pytorch_DL_nyu](./Pytorch_DL_nyu/README.md)|Pythorch/Deep learning/NYU| |✔️| |
|[SW_coding_nipa](./SW_coding_nipa/README.md)|Software coding 교육/NIPA| |✔️| |
|[RL_udemy](./RL_udemy/README.md)|Reinforcement Learning/Pytorch/TF/Udemy| |✔️| |
|[DL_from_scratch_3](./DL_from_scratch_3/README.md)|Deep Learning/Framework|✔️|| |

**Past & Another `REAMD.md`s**

- [Python Basic Study Repository - AI Robotics KR](./assets/readme/README_2019.md) → [LINK](https://github.com/ai-robotics-kr/python_study)
- [How to open `.ipynb` file in Google Colaboratory](./assets/readme/how_to_google_colab.md) - Thanks to [Hyunggi Chang](https://github.com/changh95)
